...
The Munich line of \citet{Mayr2015, Schott2019a} extended this with temporally consistent and cut-FEM-based formulations, with \citet{Mayr2015} in particular providing a clear FE counterpart to block-preconditioned monolithic Newton--Krylov FSI through Newton with GMRES and per-field ILU(0) preconditioning.
...
The standard reference here is \citet{Tukovic2018}, whose partitioned framework underpins the solids4foam toolbox \citep{Cardiff2025} and a substantial body of subsequent applied work; recent reviews~\citep{Cardiff2021} document the broader trajectory of finite volume solid mechanics that this work builds on.
Cross-ecosystem coupling frameworks such as preCICE~\citep{bungartz2016precice} extend this partitioned paradigm but remain so in nature.
...
To the authors' knowledge, no prior work has presented a body-fitted cell-centred finite volume quasi-monolithic FSI formulation solved with a Jacobian-free Newton--Krylov procedure and an approximate fluid--solid block preconditioner.
Monolithic Newton--Krylov and block-preconditioned FSI methods are well-established in the finite element literature, and a closely related staggered-FV monolithic Newton--Krylov approach exists~\citep{Eken2016}, but an analogous body-fitted cell-centred FV JFNK formulation with mesh motion treated outside the Newton vector appears to be absent.
