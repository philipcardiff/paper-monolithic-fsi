...
\begin{figure}[htbp]
    ...
    \caption{Representative fluid--solid interaction problem, where the subscript $F$ denotes fluid-domain quantities and the subscript $S$ denotes solid-domain quantities. Image adapted from \citet{OneraWebsite}.}
...
$\bb{d}_S$ denote the solid displacement from the initial configuration
$\Omega_{S,0}$ to the current configuration $\Omega_S(t)$. The deformation
...
\end{equation}
Here $\rho_{S,0}$ is the initial density and $\bb{f}_{b0,S}$ is a body force per
unit initial volume. The true (Cauchy) stress $\bb{\sigma}_S$ is supplied by the chosen
