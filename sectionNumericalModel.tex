...
\begin{eqnarray} \label{eqn:rhie_chow}
    ...
    |\bb{\Gamma}_f|
\end{eqnarray}
where $\left( \bb{\nabla} p \right)_f$ is obtained by interpolating the cell-centred least-squares pressure gradients to the face, where the pressure gradient at the centre of cell $P$ is calculated as $\left( \bb{\nabla} p \right)_P = \sum_{f \in \mathcal{F}_P} \bb{g}_{P,f} \left( p_{N_f} - p_{P} \right)$.
