...
With the adopted convention, $\hat{\bb{n}}_f \cdot \bb{d}_f > 0$ on a properly oriented mesh, and the orthogonal diffusion term in Equation~\ref{eqn:fluid_momentum_residual} represents the flux into cell $P$ from its neighbour $N_f$.
...
\end{equation}
and $\left( \bb{\nabla} \bb{v} \right)_f$ is obtained by interpolating the cell-centred least-squares velocity gradients to the face, where the velocity gradient at the centre of cell $P$ is calculated as
\begin{equation} \label{eqn:least_squares_velocity}
	\left( \bb{\nabla} \bb{v} \right)_P = \sum_{f \in \mathcal{F}_P} \bb{g}_{P,f} \otimes \left( \bb{v}_{N_f} - \bb{v}_{P} \right).
\end{equation}
...
\begin{eqnarray} \label{eqn:rhie_chow}
    ...
    |\bb{\Gamma}_f|
\end{eqnarray}
where $\left( \bb{\nabla} p \right)_f$ is obtained by interpolating the cell-centred least-squares pressure gradients to the face, where the pressure gradient at the centre of cell $P$ is calculated as in Equation \ref{eqn:fluid_momentum_residual}.
...
This is the BDF2-consistent reconstruction that ensures $\partial \bb{D}/\partial t$ at time level $n$ equals the BDF2 approximation of $\bb{v}_S^n$ from $\bb{D}^{n+1}$, $\bb{D}^{n}$ and $\bb{D}^{n-1}$.
...
\end{eqnarray}
where $\bb{T}^{\,n}_f$ and $\bb{T}^{\,n+1}_f$ are the area-weighted face force (not traction) vectors at the previous and future time levels, $\rho_0$ is the solid density in the reference configuration, $\bb{g}$ is the body force per unit mass, 
...
The displacement gradient $\left( \bb{\nabla}_0 \bb{D} \right)^{n+1}_f$ is obtained by interpolating the cell-centred least-squares displacement gradients to the face.
