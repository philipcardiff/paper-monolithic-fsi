...
\paragraph{Notation.}
For conciseness, the same symbols ($\bb{v}_F$, $\bb{v}_S$, $p_F$, $\bb{D}_S$, etc.) are used for both the continuous fields introduced in Section~\ref{sec:math_model} and their discrete cell-centred counterparts in this section.
This is a deliberate (mild) abuse of notation: in the continuous setting the symbols denote pointwise fields on $\Omega_F(t)$ or $\Omega_{S,0}$, whereas in the discrete setting they denote the corresponding cell-centred values stored at cell centroids; the intended meaning will always be clear from the context.
...
Gradients are evaluated using a first-neighbour least-squares procedure, which is exact for linear fields.
...
The same approximation is applied to the rate of change term in the solid momentum equation, but can be simplied, as the integration is performed over the initial (unchanging) reference volume:
\begin{equation}
    \left.
    \int_{\Omega_{S,0}}
...
\end{align}
where $\bb{v}_P$ and $\bb{v}_{N_f}$ are the velocities in cell $P$ and in the neighbouring cell across face $f$, $\bb{d}_f$ is the vector joining the two cell centres, and $\bb{\Gamma}_f = |\bb{\Gamma}_f|\,\hat{\bb{n}}_f$ is the outward face area vector with unit normal $\hat{\bb{n}}_f$. The linearly interpolated face velocity, used in both the convective and continuity fluxes below, is denoted $\bb{v}_f$.
...
\end{equation}
and $\left( \bb{\nabla} \bb{v} \right)_f$ is obtained by interpolating the
...
The pressure gradient $\left( \bb{\nabla} p \right)_P$ is computed using the first-neighbour least-squares gradient, while the local inertia term is discretised in time using the moving-mesh BDF2 approximation in Equation~\eqref{eqn:moving_mesh_bdf2_time_term}.
...
\end{equation}
where $\bb{v}_f$ is again the linearly interpolated face velocity, and $\mathcal{D}_{p,P}$ is the finite-volume pressure-stabilisation contribution calculated as
\begin{eqnarray} \label{eqn:rhie_chow}
	...
	\frac{\alpha_p}{a_{P_f}}
        \bigg[
	\frac{\bb{v}_{N_f} - \bb{v}_{P}}{\bb{\hat{n}}_f \cdot \bb{d}_f}
       -
       \frac{\bb{d}_f}{\bb{\hat{n}}_f \cdot \bb{d}_f} \cdot \left( \bb{\nabla} \bb{v} \right)_f
    \bigg]
    |\bb{\Gamma}_f|
\end{eqnarray}
where $a_{P_f}$ is the momentum-equation diagonal coefficient, linearly interpolated to the face.
The user-defined parameter $\alpha_p > 0$ globally scales the amount of stabilisation, and is chosen here to be 1.0 unless otherwise stated.
...
\end{equation}
and is therefore an implicit function of the current trial value of $\bb{v}_S^n$ during the nonlinear solve.
...
\end{equation}
where $\bb{T}^{\,n}_f$ and $\bb{T}^{\,n+1}_f$ are the area-weighted face force (not traction) vectors at the previous and future time levels, $\rho_0$ is the solid density in the reference configuration, $\bb{g}$ is the body force per unit mass, $c_d$ is an optional damping coefficient, and $\bb{\mathcal{D}}_{s,P}$ is a stabilisation term -- similar to $\mathcal{D}_{p,P}$ -- calculated as
\begin{eqnarray} \label{eqn:rhie_chow_s}
	...
	\alpha_d \bar{K}
        \bigg[
	\frac{\bb{D}^{n+1}_{N_f} - \bb{D}^{n+1}_{P}}{\bb{\hat{n}}_f \cdot \bb{d}_f}
       -
       \frac{\bb{d}_f}{\bb{\hat{n}}_f \cdot \bb{d}_f} \cdot \left( \bb{\nabla} \bb{D} \right)^{n+1}_f
    \bigg]
    |\bb{\Gamma}_f|
\end{eqnarray}
...
Although the Newton--Krylov operator is applied in a Jacobian-free manner, an assembled matrix is used to precondition the Krylov iterations.
This matrix is not the exact Jacobian of the coupled residual in Equation~\eqref{eqn:coupled_residual}. Instead, it is an approximate Jacobian, denoted by $\tilde{\bb{J}}$, constructed from compact finite-volume linearisations of the fluid, solid and interface-coupling terms.
Such a compact, approximate Jacobian was recently shown to efficiently precondition a second-order and higher-order cell-centred finite volume solid solvers \citep{Cardiff2026, Batistic2026}.

Using the top-level fluid--solid grouping in Equation~\eqref{eqn:coupled_residual}, the approximate Jacobian has the block form
\begin{equation}
    \tilde{\bb{J}}
    =
    \begin{bmatrix}
...
\end{equation}
where $\tilde{\bb{J}}_{ff}$ is the approximate fluid velocity--pressure block, $\tilde{\bb{J}}_{ss}$ is the approximate solid velocity block, and $\tilde{\bb{J}}_{fs}$ and $\tilde{\bb{J}}_{sf}$ approximate the interface coupling derivatives.
%This matrix is used as the preconditioning operator for the Krylov method, not as an exact linearisation of the nonlinear residual.
...
\end{equation}
where $\tilde{\bb{J}}_{mm}$ approximates the fluid momentum operator, $\tilde{\bb{J}}_{mp}$ the pressure-gradient coupling, $\tilde{\bb{J}}_{pm}$ the discrete continuity operator, and $\tilde{J}_{pp}$ the pressure-stabilisation operator.
%If the convective flux is extrapolated from previous time levels, this extrapolated flux is used in $\tilde{\bb{J}}_{mm}$.
%If the implicit-flux option is enabled, the flux is recomputed from the current trial velocity and an implicit convection contribution is included.
...
The solid block $\tilde{\bb{J}}_{ss}$ is approximated here using a compact diffusion term combined with the rate of change terms, extending the approach of displacement-based approach of \citet{cardiff2026} to a velocity formulation:
%Jacobian for the
...
\begin{eqnarray} \label{eqn:J_ss}
    \tilde{\bb{J}}_{ss}
    \approx
	\sum_{f \in \mathcal{F}_P}
	\alpha_d \frac{3 \Delta t \bar{K}}{2}
        \bigg[
	\frac{\bb{U}^{n+1}_{N_f} - \bb{U}^{n+1}_{P}}{\bb{\hat{n}}_f \cdot \bb{d}_f}
        \bigg]
        |\bb{\Gamma}_f|
        \;-\;
        \rho_S
...
%\end{equation}
where the $3\Delta t/2$ factors comes from expressing a displacement diffusion operator in terms of velocity.
%Here $K_{\mathrm{imp},f}$ is an implicit-stiffness-like face coefficient.
This approximation provides a compact (first face-neighbour), sparse solid preconditioning block, but it is not a consistent linearisation of the nonlinear total Lagrangian constitutive response.
...
\begin{equation}
    ...
    \approx
    \frac{\rho_F \nu}{\bb{\hat{n}}_f \cdot \bb{d}_f}
    |\bb{\Gamma}_f|
    \left(
    \textbf{I}
    +
    \bb{\hat{n}}_f \otimes \bb{\hat{n}}_f
    \right).
    \label{eqn:viscous_coupling_approximation}
\end{equation}
%where $\bb{\Gamma}_f$ is the interface face area vector, and $\bb{n}$ is the interface unit normal.
...
\end{equation}
Equation~\eqref{eqn:mesh_motion_pseudo_solid} is written here in compact integral form for a single fluid mesh control volume: it expresses the discrete equilibrium of the pseudo-elastic stress flux on the boundary $\Gamma_{0,f}$ of that control volume in the reference configuration, and is assembled cell-by-cell over the fluid mesh in the same cell-centred finite-volume framework as the fluid and solid momentum equations rather than as a single global statement.
...
The dimensionless scalar $\delta_f$ in Equation~\eqref{eqn:mesh_motion_pseudo_solid} is a per-face stiffness scale factor.
...
In this work, $\delta_f$ is computed from the distance from the walls $\Gamma_F$ (or a subset of the walls).
For each interior face $f$ of the fluid mesh, a base diffusivity is evaluated as the inverse square of the distance from the face centre to the nearest
moving boundary,
\begin{equation}
    \tilde{\delta}_f
    =
    \frac{1}{\ell_f^2},
    \qquad
    \ell_f = \mathrm{dist}(\bb{x}_f, \Gamma_F),
    \label{eqn:mesh_motion_diffusivity_raw}
\end{equation}
so that faces near walls carry a much larger stiffness than faces in the far field.
To keep the conditioning of the mesh motion system bounded and to avoid arbitrary scaling of the stiffness with the absolute size of the domain,
the raw values are clipped against the mesh-wide average and then normalised by that average,
\begin{equation}
    \delta_f
    =
    \frac{1}{\bar{\tilde{\delta}}}
    \,
    \min\!\left(
        c_{\max}\,\bar{\tilde{\delta}},\;
        \max\!\left(c_{\min}\,\bar{\tilde{\delta}},\,\tilde{\delta}_f\right)
    \right),
    \qquad
    \bar{\tilde{\delta}}
    =
    \frac{1}{N_f}\sum_f \tilde{\delta}_f,
    \label{eqn:mesh_motion_stiffness_scale}
\end{equation}
where $c_{\min}$ and $c_{\max}$ are user-specified clipping factors (with
defaults $c_{\min}=0.1$ and $c_{\max}=100$ used in the present work). By
construction $\delta_f$ is dimensionless and has unit mean over the fluid
mesh, so the absolute magnitudes of $\mu^\Gamma$ and $\lambda^\Gamma$ are
unchanged on average and only the spatial distribution of stiffness is
altered.
%Cell-centre values, where required (e.g.\ for output or for constructing the assembled stiffness), are obtained by face-to-cell averaging of $\delta_f$.
 The field $\delta_f$ is recomputed each time the wall-distance field is updated, i.e.\ whenever the fluid mesh moves.

After solving Equation~\eqref{eqn:mesh_motion_pseudo_solid}, the fluid mesh is updated and the mesh velocity $\bb{v}^{\Gamma}$ is obtained from the temporal change in $\bb{D}^{\Gamma}$.
This velocity is used to construct the ALE relative fluxes in the fluid residual.
The moving-mesh fluxes are constructed so as to satisfy the discrete geometric conservation law.
...
reported here; however, future work will focus on extending the approach to non-conformal interfaces using \emph{arbitrary mesh interpolation} approaches \citep{FarrellMaddison2011}.
...
Equation~\eqref{eqn:solid_future_displacement}.
...
\begin{algorithmic}[1]
\State Extract $(\bb{v}_F,p_F,\bb{v}_S)$ from the current PETSc trial vector.
