...
This section describes the discretisation and solution strategy used for the
fluid--solid interaction problem defined in Section~\ref{sec:math_model}. We
distinguish between three aspects of the method. First, the governing equations
are discretised using cell-centred finite volumes on separate fluid and solid
meshes. The fluid equations are discretised on the current ALE mesh, while the
solid equations are discretised in the reference configuration using a total
Lagrangian formulation. Second, the resulting algebraic equations are solved
using a quasi-monolithic nonlinear strategy in which the fluid velocity, fluid
pressure and solid velocity are advanced in a single coupled nonlinear solve.
Third, the method is implemented in solids4foam/OpenFOAM using PETSc SNES for
the nonlinear and linear algebra operations.

The approach is inspired by the quasi-monolithic interface treatment of
Liu et al.~\citep{Liu2014} and Algorithm~2 of Jaiman and
Joshi~\citep{JaimanJoshi2022}. As in those formulations, the fluid and solid
subproblems are coupled at the nonlinear level through the interface kinematic
and dynamic conditions. The present method differs in four main respects: the
spatial discretisation is cell-centred finite volume rather than finite
element; the fluid is solved using a standard ALE incompressible finite-volume
formulation with Rhie--Chow-type pressure stabilisation; the coupled nonlinear
unknown contains $(\bb{U}_f,p_f,\bb{U}_s)$, but not the fluid mesh displacement
or the solid displacement; and the assembled matrix supplied to the Krylov
solver is an approximate block preconditioner rather than the exact Jacobian of
the coupled residual.

The method is therefore described as quasi-monolithic. The nonlinear residual
is coupled in the fluid velocity, fluid pressure and solid velocity. However,
the fluid mesh motion and the update of the stored solid displacement are
performed outside the Newton vector.
...
The primary unknowns are stored at cell centres. Volume integrals are
approximated using midpoint quadrature,
\begin{equation}
    \int_{\Omega_P}
    \bb{\phi}
    \, d\Omega
...
\begin{equation}
    ...
    \bb{S}_f \cdot \bb{q}_f ,
    \label{eqn:fv_surface_integral}
\end{equation}
where $\bb{q}_f$ denotes the face value of $\bb{q}$. On internal faces, face
values are obtained by interpolation from the adjacent cell centres. Gradients
are evaluated using a first-neighbour least-squares procedure, which is exact
for linear fields. These finite-volume operations are applied on the current
moving mesh for the fluid equations and on the reference mesh for the total
Lagrangian solid equations.

The discretisation is constructed from nominally second-order finite-volume
operators: midpoint volume quadrature, linear face interpolation and
least-squares gradients that are exact for linear fields. The realised order of
accuracy depends on the selected convection scheme, mesh regularity, boundary
treatment and stabilisation terms, and is assessed in the numerical examples.

For the fluid equations, the convective flux is written in ALE form using the
relative face flux
\begin{equation}
    \phi_f^\mathrm{rel}
    =
...
Time derivatives are discretised using the second-order backward difference
formula. For a generic field $\bb{\phi}$,
\begin{equation}
    \left.
    \frac{\partial \bb{\phi}}{\partial t}
    \right|^{n}
    \approx
    \frac{
    3\bb{\phi}^{n}
    ...
    \label{eqn:bdf2}
\end{equation}
The same temporal discretisation is used for the fluid velocity, solid velocity and stored auxiliary fields, unless otherwise stated.
The update of the stored solid displacement, which is not an independent unknown in the coupled Newton system, is described separately in
Section~\ref{sec:auxiliary_solid_displacement_update}.
...
In this subsection, uppercase subscripts $F$ and $S$ denote fluid and solid region quantities, respectively, while lower-case subscript $f$ denotes a face
quantity. Where no ambiguity arises, the region subscript is omitted for conciseness.
...
Following the time-level convention of Liu et al.~\citep{Liu2014}, the coupled solve at time level $n$ determines $(\bb{U}_F^n,p_F^n,\bb{U}_S^n)$.
...
\end{equation}
where $\bb{U}_F$ and $p_F$ are the cell-centred fluid velocity and pressure
unknowns, and $\bb{U}_S$ is the cell-centred solid velocity unknown. The solid
...
\begin{eqnarray}
    ...
    &=&
    \begin{bmatrix}
    \bb{R}_F(\bb{U}_F,p_F;\bb{U}_S) \\
    \bb{R}_S(\bb{U}_S;\bb{U}_F,p_F)
    \end{bmatrix} \notag \\
    &=&
    \begin{bmatrix}
    \bb{R}_{F_m}(\bb{U}_F,p_F;\bb{U}_S) \\
    R_{F_p}(\bb{U}_F,p_F;\bb{U}_S) \\
    \bb{R}_S(\bb{U}_S;\bb{U}_F,p_F)
    \end{bmatrix}
    =
    \bb{0}.
...
\end{eqnarray}
Here $\bb{R}_F = \left[ \bb{R}_{F_m}; R_{F_p} \right]$ denotes the fluid equations residuals and $\bb{R}_S$ denotes the discrete solid
momentum equation, where $\bb{R}_{F_m}$ denotes the discrete fluid momentum equation and $R_{F_p}$ denotes the discrete fluid pressure equation.
The dependence of the fluid equations on $\bb{U}_S$ arises through the interface velocity and the motion of the fluid mesh boundary.
The dependence of the solid equation on $(\bb{U}_F,p_F)$ arises through the fluid traction applied to the solid interface.

Given a nonlinear trial state $(\bb{U}_F,p_F,\bb{U}_S)$, the residual is
evaluated so that both sides of the interface are driven by the current trial
solution. The current solid interface motion is first used to update the fluid
interface motion before the fluid traction is evaluated. The resulting fluid
traction is then mapped to the solid interface and applied as a traction
boundary condition in the solid equation. The current solid interface velocity
is also imposed on the fluid velocity boundary condition before the fluid
momentum and pressure equations are evaluated.

In the remainder of this subsection, the superscript $f$ denoting fluid quantities is omitted where no ambiguity arises; the subscript $f$ is reserved
for face quantities.
...
\begin{align}
    ...
    +
    \left(
    \frac{\partial \bb{U}}{\partial t}
    \right)_P
    \right],
    \label{eqn:fluid_momentum_residual}
\end{align}
where $\bb{U}_P$ and $\bb{U}_{N_f}$ are the velocities in cell $P$ and in the neighbouring cell across face $f$, $\bb{d}_f$ is the vector joining the two cell centres, $\hat{\bb{n}}_f$ is the unit face normal, and $\bb{\Gamma}_f$ is the outward face area vector.

The face summation contains the diffusion and convection contributions. The
first term is the compact orthogonal contribution to the viscous diffusion
flux. The second term is the non-orthogonal correction, where
\begin{equation}
    \bb{k}_f
    =
    \bb{\Gamma}_f
...
\end{equation}
and $\left( \bb{\nabla} \bb{U} \right)_f$ is obtained by interpolating the
cell-centred least-squares velocity gradients to the face. This correction
introduces the usual extended stencil associated with non-orthogonal
...
\end{equation}
where $\bb{U}_f$ is the linearly interpolated face velocity and $\bb{v}^{\Gamma}_f$ is the face mesh velocity.
The convected velocity $\widetilde{\bb{U}}_f$ is evaluated using a second-order upwind scheme, in which the velocity is extrapolated from the upwind cell centre using the cell-centred velocity gradient.

The remaining terms in Equation~\eqref{eqn:fluid_momentum_residual} are volume contributions.
The pressure gradient $\left( \bb{\nabla} p \right)_P$ is computed using the first-neighbour least-squares gradient, while the local inertia term is discretised in time using the BDF2 approximation in Equation~\eqref{eqn:bdf2}.
...
\begin{equation}
    ...
    -
    \sum_{f \in \mathcal{F}_P}
    \bb{\Gamma}_f \cdot \bb{U}_f
    +
    \Omega_P S_{p,P}(p;r_{AU}),
    \label{eqn:fluid_pressure_residual}
\end{equation}
Here $S_{p,P}$ is the finite-volume pressure stabilisation contribution in cell
$P$. Unless otherwise stated, this term is a Rhie--Chow-type stabilisation
constructed using $r_{AU}$, the reciprocal of an auxiliary momentum diagonal,
interpolated to the faces.

Equations~\eqref{eqn:fluid_momentum_residual} and
\eqref{eqn:fluid_pressure_residual} are written for notational simplicity as
interior-cell expressions. For fluid cells adjacent to the fluid--solid
interface, the corresponding interface-face terms are evaluated using the
kinematic interface condition. The velocity prescribed on an interface face is
obtained from the mapped solid velocity,
\begin{equation}
    \bb{U}_{\Gamma}
    =
    \mathcal{T}_{s\to f}(\bb{U}_s)
    \qquad
    \text{on } \Gamma_\mathrm{fsi},
    \label{eqn:mapped_solid_velocity}
\end{equation}
where $\mathcal{T}_{s\to f}$ denotes the solid-to-fluid interface transfer
operator. Consequently, the solid velocity enters the fluid momentum equation
through the Dirichlet boundary contribution to the viscous diffusion operator,
and it enters the pressure equation through the interface normal velocity flux.
The ALE convective flux $\phi^\mathrm{rel}_f$ on the fluid--solid interface vanishes when the fluid
interface velocity and mesh velocity are equal, consistent with the no-slip
kinematic condition.

The present implementation assumes a conformal fluid--solid interface, so the
transfer operator $\mathcal{T}_{s\to f}$ reduces to a one-to-one mapping between
corresponding solid and fluid interface faces. Extension to non-conformal
interfaces is left for future work; possible approaches include arbitrary mesh
interface methods or radial-basis-function interpolation.

For compact notation, the two fluid equations may also be grouped as
\begin{equation}
    \bb{R}_f
    =
    \begin{bmatrix}
    ...
    R_p
    \end{bmatrix},
    \label{eqn:grouped_fluid_equations}
\end{equation}
so that the coupled problem can be viewed as a top-level fluid--solid system,
\begin{equation}
    \bb{R}(\bb{\mathcal{U}})
    =
    \begin{bmatrix}
    ...
    =
    \bb{0}.
    \label{eqn:grouped_coupled_residual}
\end{equation}
This grouped notation is used below when describing the top-level
fluid--solid block preconditioner, while the separate momentum and pressure
equations are retained when describing the nested fluid velocity--pressure
split.
...
The solid displacement used to evaluate the total Lagrangian stress is a future-time quantity, denoted by $\bb{d}^{s,n+1}$, reconstructed from $\bb{U}_s^n$ and previously known solid velocities.
 Thus, during the nonlinear solve, $\bb{d}^{s,n+1}$ is an implicit function of the current trial value of $\bb{U}_s^n$.
Similar to above, the superscript $s$ denoting solid quantities is omitted where no ambiguity arises.

Since the coupled nonlinear unknown contains $\bb{U}_s^n$ rather than the solid displacement, the future-time displacement is reconstructed as
\begin{equation}
    \bb{d}^{n+1}
    =
    \bb{d}^{n}
...
For a solid control volume $P$ of initial volume $\Omega^s_{0,P}$, the discrete
solid equation is written as
\begin{equation}
    \bb{R}_{s,P}
    =
    \frac{1}{2}
    ...
    -
    \left(
    \frac{\partial \bb{U}_s}{\partial t}
    \right)_P^{n}
    -
    c_d \bb{U}_{s,P}^{n}
    \right],
...
\end{equation}
where $\rho^s_0$ is the solid density in the reference configuration,
$\bb{g}$ is the body force per unit mass and $c_d$ is an optional damping
coefficient. 
...
\begin{equation}
    ...
    \left[
    J^{n+1}_f
    \left(\bb{F}^{n+1}_f\right)^{-T}
    \cdot
    \bb{\Gamma}^{s}_{0,f}
    \right]
    \cdot
    \bb{\sigma}^{s,n+1}_f ,
    \label{eqn:solid_future_face_force}
\end{equation}
where the future-time stress  $\bb{\sigma}^{s,n+1}$ is calculated from the cell-centred $\bb{d}^{n+1}$ field (via $\bb{F}^{n+1}$) before being linearly interpolated to the faces, with an analogous definition for $\bb{f}^{\,n-1}_f$.
Here $\bb{\Gamma}^{s}_{0,f}$ is the outward reference face area vector, $\bb{F}^{*}_f$ and $J^{*}_f$ are the trial future-time deformation gradient and
Jacobian interpolated to the face, and $\bb{\sigma}^{s,*}_f$ is the corresponding Cauchy stress.
The cell-centred time derivative $\left(\partial \bb{U}_s/\partial t\right)_P$ is approximated using the BDF2 scheme in Equation~\eqref{eqn:bdf2}.

For faces on the fluid--solid interface, the solid face force is replaced by the traction obtained from the fluid side.
The mapped interface traction is computed from the current fluid pressure and viscous stress. Thus, the fluid variables enter Equation~\eqref{eqn:solid_velocity_residual} through the traction boundary condition on $\Gamma_\mathrm{fsi}$.
...
\end{equation}
where $\bb{w}$ is a Krylov vector and $\epsilon$ is the finite-difference
perturbation. Thus, the nonlinear residual evaluation described in
Section~\ref{sec:discrete_coupled_residual} defines the Newton operator used by
the Krylov solver.

Although the exact Jacobian is not assembled, preconditioning remains essential
for robustness and efficiency. An approximate block matrix is therefore
assembled and supplied as the preconditioning operator for the Krylov solve.
This matrix is not required to be a consistent linearisation of
$\bb{R}(\bb{\mathcal{U}})$; it only affects the convergence of the linear iterations, not
the converged nonlinear solution, provided the nonlinear solve reaches the
specified tolerances. The construction of this approximate block preconditioner
is described in Section~\ref{sec:approximate_block_preconditioner}.

The nonlinear iterations are terminated when the coupled residual norm and/or
the nonlinear correction satisfy prescribed tolerances. If the nonlinear solve
does not converge for a given time step, the old fluid, solid and mesh fields
are restored and the time step is retried, optionally with a reduced time-step
size.
...
linearisations of the fluid, solid and interface-coupling terms.
Such a compact, approximate Jacobian was recently shown to efficiently precondition a lower and higher order cell-centred finite volume solid solver \citep{Cardiff2026, Batistic2026}.

Using the top-level fluid--solid grouping introduced in
Equation~\eqref{eqn:grouped_coupled_residual}, the approximate Jacobian has the
block form
\begin{equation}
    \tilde{\bb{J}}
    =
...
coupling derivatives. This matrix is used as the preconditioning operator for
the Krylov method, not as an exact linearisation of the nonlinear residual.
...
$\tilde{J}_{pp}$ the pressure-stabilisation operator. If the convective flux is
extrapolated from previous time levels, this extrapolated flux is used in
$\tilde{\bb{J}}_{mm}$. If the implicit-flux option is enabled, the flux is
recomputed from the current trial velocity and an implicit convection
contribution is included.

This block form also permits nested velocity--pressure preconditioning within
the fluid block. In particular, the exact pressure Schur complement associated
with Equation~\eqref{eqn:fluid_block_preconditioner} is
\begin{equation}
    \bb{S}_p
    =
    \tilde{J}_{pp}
    ...
    \tilde{\bb{J}}_{mm}^{-1}
    \tilde{\bb{J}}_{mp}.
    \label{eqn:pressure_schur_complement}
\end{equation}
In practice, this Schur complement is replaced by an inexpensive approximation
based on the finite-volume pressure equation and pressure-stabilisation
operator. This provides a nested split strategy in which the top-level
preconditioner separates fluid and solid fields, while the fluid block is
itself approximately split into velocity and pressure components.

The solid block $\tilde{\bb{J}}_{ss}$ is a compact approximate Jacobian for the
velocity-form total Lagrangian solid equation. It is based on the type of
segregated finite-volume solid mechanics operator commonly used for implicit
solid solves, rather than on a full consistent tangent of the nonlinear
stress-divergence equation. In compact form, it may be written as
\begin{equation}
    \tilde{\bb{J}}_{ss}
    \approx
    \bb{\nabla} \cdot
    \left[
    (3/2) K_{\mathrm{imp},f} \Delta t
    \bb{\nabla} \bb{U}_s
    \right]
    -
    \rho_s
    \frac{\partial \bb{U}_s}{\partial t},
    \label{eqn:solid_approximate_jacobian}
\end{equation}
with optional damping contributions. Here $K_{\mathrm{imp},f}$ is an
implicit-stiffness-like face coefficient. This approximation provides a
compact, sparse solid preconditioning block, but it is not a consistent
linearisation of the nonlinear total Lagrangian constitutive response.
Details of this approach are provided in \citet{Cardiff2026}.

The off-diagonal block $\tilde{\bb{J}}_{fs}$ approximates the dependence of the
fluid momentum and pressure equations on the solid velocity at the interface.
Since the fluid interface velocity is imposed through a prescribed value boundary
condition, this block represents the effect of the mapped solid interface
velocity on the adjacent fluid equations. The momentum contribution arises from
the boundary part of the viscous operator, while the pressure-equation
contribution arises from the normal velocity flux through the interface.

The off-diagonal block $\tilde{\bb{J}}_{sf}$ approximates the dependence of the
solid equation on the fluid traction. More explicitly, this block contains the
approximate derivatives with respect to the fluid velocity and pressure,
\begin{equation}
    \tilde{\bb{J}}_{sf}
    =
    \begin{bmatrix}
    \tilde{\bb{J}}_{sm} & \tilde{\bb{J}}_{sp}
    \end{bmatrix}.
    \label{eqn:solid_fluid_coupling_block}
\end{equation}
The pressure contribution $\tilde{\bb{J}}_{sp}$ assumes that the pressure
traction on each solid interface face is determined by the adjacent fluid
cell-centre pressure, with the density factor required by the kinematic
pressure convention. When viscous-stress transfer is enabled, a compact
normal-gradient approximation is also included in $\tilde{\bb{J}}_{sm}$ for
the dependence of the fluid viscous traction on the adjacent fluid velocity,
\begin{equation}
    \frac{\partial F_i}{\partial U_k}
    \approx
    \rho_f \nu_\mathrm{eff}
    \delta_f
    |\bb{S}_f|
...
\end{equation}
where $\delta_f$ is the face normal-gradient coefficient, $\bb{S}_f$ is the
interface face area vector, and $\bb{n}$ is the interface unit normal.
...
Section~\ref{sec:discrete_coupled_residual}. This distinction is important:
the approximate Jacobian in Equation~\eqref{eqn:block_preconditioner} affects
the efficiency and robustness of the Krylov iterations, but it does not define
the nonlinear equations being solved.

%In the implementation, $\tilde{\bb{J}}$ is first assembled as a nested block
%matrix and is then converted to a monolithic sparse matrix before being passed
%to the linear solver. This allows standard algebraic preconditioners and
%field-split strategies to be applied to the coupled system.
...
The interior fluid mesh displacement is computed using a pseudo-solid
linear-elastic problem posed on the fluid reference mesh,
\begin{equation}
    \oint_{\Gamma^f_0}
    \hat{\bb{n}}^f_0
    \cdot
...
parameters used only to control the distribution of the mesh motion. They do
not represent material properties of the fluid.

In this work, the artificial stiffness is increased near the fluid--solid
interface by scaling it inversely with the square of the distance from the
interface. This makes the mesh comparatively stiff near the moving boundary and
more compliant farther away, reducing distortion of interface-adjacent cells
while allowing the displacement to decay smoothly through the fluid domain.

After solving Equation~\eqref{eqn:mesh_motion_pseudo_solid}, the fluid mesh is
updated and the mesh velocity $\bb{v}^{\Gamma}$ is obtained from the temporal
change in $\bb{d}^{\Gamma}$. This velocity is used to construct the ALE
relative fluxes in the fluid residual. The moving-mesh fluxes are constructed
so as to satisfy the discrete geometric conservation law.
...
The method is implemented in solids4foam~\citep{Cardiff2025}, an
OpenFOAM-based finite-volume toolbox, with PETSc used for the nonlinear and
linear algebra operations. The coupled FSI algorithm is implemented through a
fluid--solid coupling interface, which calls separate fluid and solid region
models to evaluate the equation blocks and assemble the approximate Jacobian
blocks used for preconditioning.

In the implementation, the coupled PETSc field is named \texttt{UpU}. The local
vector layout consists of a contiguous fluid block followed by a contiguous
solid block. In two dimensions, the fluid cell block is ordered as
$(U_{fx},U_{fy},p_f)$ and the solid cell block as $(U_{sx},U_{sy})$. In three
dimensions, the corresponding blocks are
$(U_{fx},U_{fy},U_{fz},p_f)$ and $(U_{sx},U_{sy},U_{sz})$. PETSc index sets are
constructed for the full fluid block, the fluid velocity sub-block, the fluid
pressure sub-block and the solid block. These index sets are used to extract
sub-vectors during equation evaluation and to define block and nested
field-split preconditioners.

The nonlinear residual is evaluated through a callback supplied to PETSc SNES.
For each nonlinear trial vector, the trial values are copied into the
corresponding OpenFOAM fields. Boundary conditions and interface mappings are
then updated, and the fluid momentum equation, fluid pressure equation and
solid momentum equation are inserted into the PETSc residual vector. In typical
calculations, PETSc applies the Newton--Krylov operator in a matrix-free manner,
while the approximate Jacobian $\tilde{\bb{J}}$ described in
Section~\ref{sec:approximate_block_preconditioner} is supplied as the
preconditioning matrix.

The approximate Jacobian is first assembled as a PETSc \texttt{MatNest},
\begin{equation}
    \tilde{\bb{J}}
    =
    \begin{bmatrix}
    ...
    \end{bmatrix},
\end{equation}
and is then converted to a monolithic AIJ sparse matrix. This conversion allows
standard PETSc preconditioners, including LU, ILU, additive Schwarz, block
Jacobi and field-split methods, to be applied to the coupled system. When a
nested fluid split is used, the fluid block $\tilde{\bb{J}}_{ff}$ is further
split into velocity and pressure sub-blocks, enabling approximate
Schur-complement preconditioning of the fluid velocity--pressure system.

The interface transfer is currently performed using a direct conformal map
between the fluid and solid interface patches. The solid interface displacement
and velocity are mapped to the fluid mesh-motion and velocity boundary
conditions. The fluid pressure traction and, when enabled, the viscous traction
are mapped back to the solid traction boundary condition. The implementation
therefore assumes a single conformal fluid--solid interface for the calculations
reported here; however, future work will focus on extending the approach to non-conformal interfaces using \emph{arbitrary mesh interpolation} approaches \citep{https://doi.org/10.1016/j.cma.2010.07.015}.

The time-step procedure is summarised in Algorithm~\ref{alg:quasi_monolithic_fsi}.
The residual callback used inside the nonlinear solve is summarised in
Algorithm~\ref{alg:coupled_residual}.
...
\State Predict the solid displacement using
Equation~\eqref{eqn:solid_displacement_prediction}.
\State Compute the solid displacement gradient and interpolate solid
cell-centre displacements to the solid interface points.
...
\State Construct the fluid flux for the convection term, either by
extrapolation from previous time levels or from the current trial velocity.
...
$(\bb{U}_f,p_f,\bb{U}_s)$ using PETSc SNES.
...
retry the time step, optionally with a reduced time step.
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
\State Extract $(\bb{U}_f,p_f,\bb{U}_s)$ from the current PETSc trial vector.
