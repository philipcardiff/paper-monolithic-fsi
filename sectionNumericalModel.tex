...
In this subsection, uppercase subscripts $F$ and $S$ denote fluid- and solid-region quantities, respectively, and the lower-case subscript $f$ is reserved for face quantities. Where no ambiguity arises, the region subscript is omitted for conciseness. The trial state appearing in the residual arguments below is understood at time level~$n$.
...
The interface coupling is represented by the off-block arguments: $\bb{U}_S$ enters the fluid equations through the interface velocity boundary condition and the motion of the fluid mesh boundary, while $(\bb{U}_F,p_F)$ enters the solid equation through the fluid traction applied to the solid interface.
...
In the remainder of this subsection, the region subscript $F$ on fluid quantities is omitted where no ambiguity arises; the lower-case subscript $f$ continues to denote a face quantity.
...
\end{align}
where $\bb{U}_P$ and $\bb{U}_{N_f}$ are the velocities in cell $P$ and in the neighbouring cell across face $f$, $\bb{d}_f$ is the vector joining the two cell centres, and $\bb{\Gamma}_f = |\bb{\Gamma}_f|\,\hat{\bb{n}}_f$ is the outward face area vector with unit normal $\hat{\bb{n}}_f$. The linearly interpolated face velocity, used in both the convective and continuity fluxes below, is denoted $\bb{U}_f$.
...
\end{equation}
where $\bb{U}_f$ is again the linearly interpolated face velocity, and $S_{p,P}$ is the finite-volume pressure-stabilisation contribution in cell~$P$. Unless otherwise stated, $S_{p,P}$ is a Rhie--Chow-type stabilisation constructed from a face-interpolated coefficient $r_{AU}$, taken as the reciprocal of an auxiliary momentum-equation diagonal coefficient. Its detailed construction is given in Section~\ref{sec:approximate_block_preconditioner}.

Equations~\eqref{eqn:fluid_momentum_residual} and~\eqref{eqn:fluid_pressure_residual} are written as interior-cell expressions. For fluid cells adjacent to the fluid--solid interface, the corresponding interface-face terms use the kinematic interface condition: the prescribed interface velocity is obtained from the mapped solid velocity,
\begin{equation}
    \bb{U}_{\Gamma}
    =
...
In the remainder of this subsection, the region subscript $S$ on solid quantities is omitted where no ambiguity arises. Because the coupled unknown contains the solid velocity $\bb{U}_S^n$ rather than the solid displacement, the future-time displacement $\bb{d}^{n+1}$ required to evaluate the total Lagrangian stress is reconstructed from $\bb{U}_S^n$ and stored solid velocities as
\begin{equation}
    \bb{d}^{n+1}
    =
    \bb{d}^{n}
...
\end{equation}
and is therefore an implicit function of the current trial value of $\bb{U}_S^n$ during the nonlinear solve.

For a solid control volume $P$ of initial volume $\Omega_{0,P}$, the discrete
...
\end{equation}
where $\bb{t}^{\,n}_f$ and $\bb{t}^{\,n+1}_f$ are the area-weighted face force vectors at the previous and future time levels, $\rho_0$ is the solid density in the reference configuration, $\bb{g}$ is the body force per unit mass and $c_d$ is an optional damping coefficient.
...
\end{equation}
where $\bb{\Gamma}_{0,f}$ is the outward reference face area vector, $\bb{F}^{n+1}_f$ and $J^{n+1}_f = \det \bb{F}^{n+1}_f$ are the future-time deformation gradient and volume ratio interpolated to the face, and $\bb{\sigma}^{n+1}_f$ is the corresponding Cauchy stress. The cell-centred future-time stress is computed from the reconstructed displacement $\bb{d}^{n+1}$ (via $\bb{F}^{n+1}$) and then linearly interpolated to the faces; the previous-time face force $\bb{t}^{\,n}_f$ is defined analogously from stored solid fields. The cell-centred time derivative $\left(\partial \bb{U}/\partial t\right)_P$ is approximated using the BDF2 scheme in Equation~\eqref{eqn:bdf2}.

For faces on the fluid--solid interface, the solid face force is replaced by the area-weighted traction mapped from the fluid side, computed from the current fluid pressure and viscous stress. The fluid variables thus enter Equation~\eqref{eqn:solid_velocity_residual} through this traction boundary condition on $\Gamma_\mathrm{fsi}$.
...
Using the top-level fluid--solid grouping in
Equation~\eqref{eqn:coupled_residual}, the approximate Jacobian has the
block form
\begin{equation}
    \tilde{\bb{J}}
    =
