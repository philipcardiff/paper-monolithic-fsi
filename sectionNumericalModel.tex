...
Recall from Section~\ref{sec:fv_discretisation} that $\mathcal{F}_P$ denotes the set of faces of cell $P$, with $\bb{\Gamma}_f = |\bb{\Gamma}_f|\, \hat{\bb{n}}_f$ the outward face area vector and $\bb{d}_f$ the vector joining the centroids of $P$ and its neighbour $N_f$ across face $f$.
...
\end{align}
where $\bb{v}_P$ and $\bb{v}_{N_f}$ are the velocities in cell $P$ and in the neighbouring cell across face $f$.
The linearly interpolated face velocity, used in both the convective and continuity fluxes below, is denoted $\bb{v}_f$.

The face summation contains the diffusion and convection contributions. The first term is the compact orthogonal contribution to the viscous diffusion
