...
Following the time-level convention of Liu et al.~\citep{Liu2014}, the coupled solve at time level $n$ determines $(\bb{U}_f^n,p^n,\bb{U}_s^n)$.
...
\begin{eqnarray}
    ...
    =
    \begin{bmatrix}
    \bb{R}_m(\bb{U}_f,p_f;\bb{U}_s) \\
    R_p(\bb{U}_f,p_f;\bb{U}_s) \\
    \bb{R}_s(\bb{U}_s;\bb{U}_f,p_f)
    \end{bmatrix}
    =
    \bb{0}.
...
\end{eqnarray}
Here $\bb{R}_f = \left[ \bb{R}_m; R_p \right]$ denotes the fluid equations residuals and $\bb{R}_s$ denotes the discrete solid
momentum equation, where $\bb{R}_m$ denotes the discrete fluid momentum equation and $R_p$ denotes the discrete fluid pressure equation.
...
a nonlinear algebraic system for the unknown vector $\bb{x}$. This system is
solved using a Newton--Krylov method. At nonlinear iteration $k$, the Newton
correction $\delta \bb{x}_k$ formally satisfies
\begin{equation}
    \bb{J}(\bb{x}_k)
    \delta \bb{x}_k
    =
...
\begin{equation}
    ...
    =
    \left.
    \frac{\partial \bb{R}}{\partial \bb{x}}
    \right|_{\bb{x}_k}
\end{equation}
...
$\bb{R}(\bb{x})$; it only affects the convergence of the linear iterations, not
