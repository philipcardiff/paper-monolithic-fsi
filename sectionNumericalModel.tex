...
%\subsection{Discrete coupled residual}\text{d}\Omega
...
We distinguish between three aspects of the method. First, the governing equations are discretised using cell-centred finite volumes.
...
The present method differs in three main respects: the spatial discretisation is cell-centred finite volume rather than finite element; the fluid is solved using a standard ALE incompressible finite-volume formulation with Rhie--Chow-type pressure stabilisation; and the assembled matrix supplied to the Krylov solver is an approximate block preconditioner rather than the exact Jacobian of the coupled residual.
...
The primary unknowns are stored at cell centres and represent the pointwise values at those locations. \hl{do we mention equivalence with cell means?}
...
through face $f$ is defined by Equation~\eqref{eqn:relative_flux}.
%\begin{equation}
%    \phi^\mathrm{rel}_f
%    =
%    \bb{\Gamma}_f \cdot
...
Algebraically, $\tilde{J}_{pp}$ approximates the pressure coupling that arises through the velocity block when the velocity is eliminated from the coupled momentum--pressure system. This motivates using $\tilde{J}_{pp}$ as a cheap surrogate for the formal pressure Schur complement,
\begin{equation}
\tilde{S}_p = \tilde{J}_{pp} - \tilde{\bb{J}}_{pm}\tilde{\bb{J}}_{mm}^{-1}\tilde{\bb{J}}_{mp},
\label{eqn:schur_complement}
\end{equation}
within the velocity--pressure split. Forming $\tilde{S}_p$ explicitly would require inversion of $\tilde{\bb{J}}_{mm}$ and is avoided.
...
split into velocity and pressure components using the approximate Schur-complement strategy described in Section~\ref{sec:approximate_block_preconditioner}.
...
The fluid pressure and viscous traction are mapped back to the solid traction boundary condition.
The calculations reported here therefore assume a single conformal fluid--solid interface, as noted in Section \ref{sec:discrete_coupled_residual}.
...
%		\cdot \left[\nu (\bb{F}^\Gamma)^{-T} \cdot \left( \bb{\nabla}_0 \bb{v}^f \right) \right] \, \text{d}\Gamma^f_0
%	+ \int_{\Omega^f_0} J^\Gamma (\bb{F}^\Gamma)^{-T} \cdot \left( \bb{\nabla}_0 p \right) \, \text{d}\Omega^f_0 \notag \\
%	&&+ \int_{\Omega^f_0} \bb{f}_{b_0}^f \, \text{d}\Omega^f_0
%	- \int_{\Omega^f_0} J^\Gamma \frac{\partial \bb{v}^f}{\partial t} \, \text{d}\Omega^f_0 
%	- \oint_{\Gamma^f_0}  \left[ J^\Gamma (\bb{F}^\Gamma)^{-\text{T}} \cdot \hat{\bb{n}}_0^f \right] \cdot \left(\bb{v}^f - \frac{\partial \bb{d}_m}{\partial t}\right) \bb{v}^f \, \text{d}\Gamma^f_0
%	\notag \\
%	&&+\; \bb{\mathcal{D}}_v
%	\notag \\
%    R_p %(\bb{v})
%    &=&	\oint_{\Gamma^f_0}  \left[ J^\Gamma (\bb{F}^\Gamma)^{-\text{T}}  \cdot \hat{\bb{n}}_0^f \right] \cdot \bb{v}^f \, \text{d}\Gamma^f_0 \;+\; \mathcal{D}_p
%    \notag \\
%    \bb{R}_m
%    &=&
...
%    \bb{R}_d %(\bb{v})
%    &=&
%    \oint_{\Gamma_0^s} \left( J \bb{F}^{-\text{T}} \cdot \bb{n}_0^s \right) \cdot \bb{\sigma}^s \ \text{d}\Gamma_0^s
%    + \int_{\Omega_0^s}  \bb{f}_{b_0}^s \, \text{d}\Omega_0^s
%    - \int_{\Omega_0^s} \rho_0 \frac{\partial^2 \bb{d} }{\partial t^2} \; \text{d}\Omega_0^s
%    \;+\; \bb{\mathcal{D}}_d
%\end{eqnarray}
...
%\end{eqnarray}
%where subscript $P$ indicates a value at the centroid of the cell $P$, $\Omega_P$ is the volume of cell $P$ and $\int_{\Omega_P} (\bb{x} - \bb{x}_P) \text{d}\Omega_P \equiv 0$ by definition of the cell centroid.
...
%	\oint_{\Gamma_P} \bb{n} \cdot \bb{q}  \; \text{d}\Gamma_P
...
%%	\oint_{\Gamma_P} \bb{n} \cdot \left(\bb{v} - \frac{\partial \bb{d}_{m}}{\partial t}\right) \bb{v} \; \text{d}\Gamma_P
...
%	\int_{\Omega^f_0} \bb{f}_{b_0}^f \, \text{d}\Omega^f_0	&\approx&		\left( \bb{f}_{b_0}^f \right)_P  \Omega_{0,P} \\
%	\int_{\Omega^f_0} J^\Gamma \frac{\partial \bb{v}^f}{\partial t} \, \text{d}\Omega^f_0
...
%\begin{align}
%	\oint_{\Gamma^f_0}  \left[ J^\Gamma (\bb{F}^\Gamma)^{-\text{T}} \cdot \hat{\bb{n}}_0^f \right]
%		& \cdot \left(\bb{v}^f - \frac{\partial \bb{d}_m}{\partial t}\right) \bb{v}^f \, \text{d}\Gamma^f_0
%		\approx \notag \\
...
%\begin{eqnarray}
%    R_p    &=&	\oint_{\Gamma^f_0}  \left[ J^\Gamma (\bb{F}^\Gamma)^{-\text{T}}  \cdot \hat{\bb{n}}_0^f \right] \cdot \bb{v}^f \, \text{d}\Gamma^f_0 \;+\; \mathcal{D}_p
%\end{eqnarray}
%
%The velocity divergence term is approximated for cell $P$ as
%\begin{eqnarray}
%	\oint_{\Gamma^f}  \left[ J_m \bb{F}_m^{-\text{T}}  \cdot \hat{\bb{n}}_0 \right] \cdot \bb{v} \, \text{d}\Gamma_0
%	&\approx&
%		\sum_{f_i \in \mathcal{F}^{\text{int}}_P}
...
%%	\tilde{\bb{J}}_{dd}
%%	&=&
%%	\frac{\partial}{\partial \bb{u}} \left[ \oint_{\Gamma_P} \alpha \bar{K} \, \bb{n} \cdot \bb{\nabla} \bb{u} \; \text{d}\Gamma_P
%%	 \; -\;  \int_{\Omega_P} \rho \frac{\partial^2 \bb{u} }{\partial t^2} \, \text{d}\Omega_P \right]
%%\end{eqnarray}
