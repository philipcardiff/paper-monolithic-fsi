...
The fluid and solid domains are discretised using separate cell-centred
finite-volume meshes. The set of fluid control volumes is denoted by
$\mathcal{P}^f$ and the set of solid control volumes by $\mathcal{P}^s$. A
generic cell is denoted by $P$, with volume $V_P$ and boundary faces
$f \in \mathcal{F}_P$. The outward face area vector associated with face $f$ is
denoted by $\bb{S}_f$.
...
\begin{equation}
    ...
    \label{eqn:bdf2}
\end{equation}
The same temporal discretisation is used for the fluid velocity, pressure
where required, solid velocity and stored auxiliary fields, unless otherwise
stated. The update of the stored solid displacement, which is not an
independent unknown in the coupled Newton system, is described separately in
...
fluid velocity, fluid pressure and solid velocity. These are collected into the
unknown vector
\begin{equation}
    \bb{x}
    =
...
they are not independent unknowns in $\bb{x}$.
...
\end{equation}
Here $\bb{R}_m$ denotes the discrete fluid momentum equation, $R_p$ denotes the
discrete fluid pressure equation, and $\bb{R}_s$ denotes the discrete solid
momentum equation. The dependence of the fluid equations on $\bb{U}_s$ arises
through the interface velocity and the motion of the fluid mesh boundary. The
dependence of the solid equation on $(\bb{U}_f,p_f)$ arises through the fluid
traction applied to the solid interface.
...
For a fluid control volume of volume $V$, the discrete momentum equation has
the form
\begin{equation}
    \bb{R}_m
    =
    ...
    \bb{\nabla} p_f
    -
    \frac{\partial \bb{U}_f}{\partial t}
    -
    \bb{\nabla} \cdot
    \left(
    \phi^\mathrm{rel} \bb{U}_f
    \right)
    +
    \bb{S}_m
    \right],
...
\end{equation}
where $\nu_\mathrm{eff}$ is the effective kinematic viscosity,
$\phi^\mathrm{rel}$ is the ALE relative flux and $\bb{S}_m$ denotes any
additional momentum stabilisation. The discrete pressure equation is
\begin{equation}
    R_p
    =
    V
...
\end{equation}
where $S_p$ is the finite-volume pressure stabilisation operator and
$r_{AU,f}$ is obtained from the reciprocal of an auxiliary momentum diagonal.
Unless otherwise stated, $S_p$ is a Rhie--Chow-type pressure stabilisation.
...
The solid equation is evaluated using the trial solid velocity and the
future-time displacement reconstructed in
Equation~\eqref{eqn:solid_future_displacement}. The deformation gradient,
stress and face forces are evaluated from this reconstructed displacement. For
a solid control volume of volume $V$, the discrete solid equation has the form
\begin{equation}
    \bb{R}_s
    =
    V
...
\end{equation}
where $\bb{f}^{\,n-1}$ and $\bb{f}^{\,*}$ are the old and trial future-time
internal force fields, $\rho_s$ is the solid density, $\bb{g}$ is the body
force and $c_d$ is an optional damping coefficient. The force field
$\bb{f}^{\,*}$ is computed from the stress field evaluated using the
trial future-time displacement. The interface traction from the fluid enters
Equation~\eqref{eqn:solid_velocity_residual} through the traction boundary
condition on $\Gamma_\mathrm{fsi}$.

The fluid and solid equation blocks may be multiplied by separate scale factors
before forming the global coupled residual. These scale factors are used to
improve conditioning and to balance the nonlinear residual norm. They do not
alter the unscaled physical equations.
